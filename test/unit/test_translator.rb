...
class TestTranslator < Test::Unit::TestCase
  ...
  def setup
    @query = "rft.genre=article&rft.title=journal%20of%20something&rft.issn=1234-1234&rft.volume=17&rft.issue=1" +
              "&rft.atitle=Article%20title&my_param=A%20New%20value" 
    
    @citation = Cedilla::Translator.query_string_to_citation("openurl", @query)
...
# --------------------------------------------------------------------------------------------------------  
  def test_query_string_to_citation
    ...
    assert citation.issn.nil?, "Should have returned an empty citation when the query string was empty and no translation file was provided!"
      
    # Test that no translation file tries to match values directly from the citation object's property names
    citation = Cedilla::Translator.query_string_to_citation("nothing", "#{@query}&issn=0000-0001")
    assert_equal "0000-0001", citation.issn, "Should have returned the ISSN for 'issn' instead of 'rft.issn' because NO translation file was provided! #{citation.issn}"
    ...
    citation = Cedilla::Translator.query_string_to_citation("openurl", "#{@query}&issn=0000-0001")
...
# --------------------------------------------------------------------------------------------------------
  def test_citation_to_query_string
    ...
    out = Cedilla::Translator.citation_to_query_string("nothing", @citation)
    ...
    out = Cedilla::Translator.citation_to_query_string("openurl", @citation)
    assert_equal "article", @citation.genre
